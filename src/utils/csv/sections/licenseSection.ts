...
export const createLicenseSection = (title: string, licenses: RawLicense[]): CSVSection => {
  ...
  const rows = licenses.map(license => {
    ...
    const total = license.TotalLicenses ?? license.AllowedLicenses ?? -1;
    const used = license.UsedLicenses ?? 0;
    const available = total === -1 ? 'Unlimited' : 
      ((total || 0) - (used || 0)).toString();
    ...
    return [name, total === -1 ? 'Unlimited' : total.toString(), used.toString(), available, usagePercentage];
  });
